...
def build_model():
    ...
    print(f"linear_features: {linear_features}")
    print(f"dnn_features: {dnn_features}")
    ...
    linear_optimizer = tf.keras.optimizers.Ftrl(l1_regularization_strength=5)
    ...
    dnn_dense_layer1 = tf.keras.layers.Dense(units=1024, activation='relu')
    dnn_dense_layer2 = tf.keras.layers.Dense(units=512, activation='relu')
    dnn_dense_layer3 = tf.keras.layers.Dense(units=256, activation='relu')
    dnn_dense_layer4 =  tf.keras.layers.Dense(units=1)

    dnn_input_layer = build_input_layer()
    output = dnn_feature_layer(dnn_input_layer)
    output = dnn_norm_layer(output)
    output = dnn_dense_layer1(output)
    output = dnn_dense_layer2(output)
    output = dnn_dense_layer3(output)
    output = dnn_dense_layer4(output)

    dnn_model =  tf.keras.Model(inputs=list(dnn_input_layer.values()), outputs=[output])
    dnn_optimizer = tf.keras.optimizers.Adagrad()

    # wide&deep
    wide_deep_model = tf.keras.experimental.WideDeepModel(linear_model, dnn_model)
    wide_deep_model.compile(optimizer=[linear_optimizer,dnn_optimizer],
                            loss='mae',
                            metrics=['mae', 'mse', tf.keras.metrics.AUC()])
